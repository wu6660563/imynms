<%@page import="com.mynms.performance.model.PerformanceDBInfo"%>
<%@page import="com.mynms.common.util.page.Page"%>
<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
<%
    String path = request.getContextPath();
    String basePath = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+path+"/";
    List<PerformanceDBInfo> list = (List<PerformanceDBInfo>) request.getAttribute("list");
...
<html>
    <head>
    	...
    	<meta http-equiv="description" content="数据库性能信息">
            ...
            <div id="page" data-role="page" style="height: 100%">
                <div style="padding: 5px 15px; " data-role="content">
                    <ul id="listview" data-role="listview" data-inset="true">
                        <li data-role="list-divider" class="list-divider">数据库性能信息</li>
                        ...
                        if (list != null && list.size() > 0) {
                        	for (PerformanceDBInfo info : list) {
                        %>
                        <li title="content">
                            <table style="width: 100%;text-align: left;">
                                <tr>
                                    <td width="40%">名称：</td><td width="60%"><%=info.getName()%></td>
                                </tr>
                                <tr>
                                    <td width="40%">IP地址：</td><td width="60%"><%=info.getIpAddress()%></td>
                                </tr>
                                <tr>
                                    <td width="40%">类型：</td><td width="60%"><%=info.getSubtype()%></td>
                                </tr>
                                <tr>
                                    <td width="40%">连通率：</td><td width="60%"><%=info.getPing()%>%</td>
                                </tr>
                            </table>
                        </li>
                        <%
                ...
                <script type="text/javascript">
                    ...
                    var scrollPagination = new ScrollPagination({
                        ...
                        'url' : '<%=path%>' + '/performanceDBInfo/list.do',
                        'scrollTarget' : $(window),
                        'heightOffset' : 10,
                        'beforeLoad' : function() {
                    ...
                    function search() {
			            ...
			            mainForm.action = "<%=path%>/performanceDBInfo/list.do";
			            mainForm.submit();
			        }
                </script>
            </div>
